...
class TestLDABasic:
    ...
    def test_fit_lda_basic(self, multiclass_data):
        ...
        lda = fit_lda(multiclass_data, target='class')
...
class TestLDAPrediction:
    ...
    def test_score(self, multiclass_data):
        """Test scoring"""
        lda = fit_lda(multiclass_data, target='class')
        accuracy = lda.score(multiclass_data)
        
        assert 0 <= accuracy <= 1
        assert accuracy > 0.7  # Should fit training data reasonably
...
class TestLDAVisualization:
    ...
    def test_visualize_binary(self, binary_data, monkeypatch):
        ...
        lda = fit_lda(binary_data, target='class')
        lda.visualize()
...
class TestLDASolvers:
    ...
    def test_svd_solver(self, multiclass_data, monkeypatch):
        ...
        lda = fit_lda(multiclass_data, target='class', solver='svd')
    ...
    def test_lsqr_solver(self, multiclass_data, monkeypatch):
        ...
        lda = fit_lda(multiclass_data, target='class', solver='lsqr')
